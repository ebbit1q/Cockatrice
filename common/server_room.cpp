...
Server_Room::~Server_Room()
{
    ...
    QWriteLocker gamesLocker(&gamesLock);
    for (auto *game : games.values())
        delete game;
    games.clear();

    QWriteLocker usersLocker(&usersLock);
    users.clear();
}
...
Server_Room::getInfo(ServerInfo_Room &result, bool complete, bool showGameTypes, bool includeExternalData) const
{
    ...
    {
        QReadLocker gamesLocker(&gamesLock);
    ...
    {
        QReadLocker usersLocker(&usersLock);
...
void Server_Room::say(const QString &userName, const QString &userMessage, bool sendToIsl)
{
    ...
    if (chatHistorySize != 0) {
        ...
        QWriteLocker historyLocker(&historyLock);
